...
class ExpiredUserRemovalService:
    ...
    def mark_removal_pending(df: pd.DataFrame, index,) -> None:
        ...
        df.at[index, SubscriptionColumn.UPDATED_AT,] = datetime.now()

    @staticmethod
    def mark_removed(df: pd.DataFrame, index,) -> None:
        df.at[index, SubscriptionColumn.STATUS,] = "removed"
        df.at[index, SubscriptionColumn.REMOVED_AT,] = datetime.now()
        df.at[index, SubscriptionColumn.UPDATED_AT,] = datetime.now()

    @staticmethod
    def mark_removal_failed(df: pd.DataFrame, index, error: Exception,) -> None:
        ...
        if pd.isna(existing_error) or existing_error == "": df.at[index, error_column] = error_message
        else: df.at[index, error_column] = f"{existing_error}\n{error_message}"
        df.at[index, SubscriptionColumn.STATUS] = "removal_failed"
        df.at[index, SubscriptionColumn.UPDATED_AT] = datetime.now()

    @staticmethod
    def remove_reminders(reminders_df: pd.DataFrame, subscription_id: int,) -> pd.DataFrame:
    ...
    async def remove_expired_users(cls,
        ...
        return (result_subscriptions, result_members, result_reminders,)
