...
class ReminderService:
    ...
    def get_enabled_configs(reminder_config_df: pd.DataFrame) -> pd.DataFrame:
        return reminder_config_df[
            reminder_config_df[ReminderConfigColumn.ENABLED]
            .astype(str)
            .str.upper()
            .eq("TRUE")
        ]
    ...
    def get_next_reminder_id(reminders_df: pd.DataFrame) -> int:
        ...
        ids = pd.to_numeric(
            reminders_df[ReminderColumn.ID],
            errors="coerce",
        ).dropna()
    ...
    def reminder_exists(
        ...
        return (
            (
                reminders_df[ReminderColumn.SUBSCRIPTION_ID].astype(str)
                == str(subscription_id)
            )
            & (
                reminders_df[ReminderColumn.REMINDER_DAYS].astype(str)
                == str(reminder_days)
            )
        ).any()
    ...
    def create_reminders(
        ...
        for _, subscription in subscriptions_df.iterrows():
            try:
                reminders, next_reminder_id = cls.process_subscription(
